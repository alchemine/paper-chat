...
from paper_chat.agents.retrieval_agent_builder import (
    ReactRetrievalAgentBuilder,
    SelfRetrievalAgentBuilder,
)
...
class RetrievalAgentExecutor:
    def __init__(self, llm=CHAT_LLM, retrieval_agent: str = "react") -> None:
        self._llm = llm
    ...
    def build(self, arxiv_url: str) -> None:
        ...
        kwargs = {"memory": self._memory, "summary": self._summary, "docs": self._docs}
        match self._retrieval_agent:
            case "react":
                builder = ReactRetrievalAgentBuilder(**kwargs)
            case "self":
                # NOTE: does not have memory
                builder = SelfRetrievalAgentBuilder(**kwargs)
            case _:
                raise ValueError(f"Invalid retrieval agent: {self._retrieval_agent}")

        self._agent_executor = builder.build()
    ...
    def stream(self, prompt: str) -> dict:
        ...
        # usage_metadata = {}
        # for step in self._agent_executor.stream(
        #     {"question": prompt}, config=self._config
        # ):
        #     if "retrieve" in step:
        #         retrieve = step["retrieve"]
        #         question = retrieve["question"]
        #         documents = retrieve["documents"]
        #     elif "grade_documents" in step:
        #         grade_documents = step["grade_documents"]
        #         question = grade_documents["question"]
        #         documents = grade_documents["documents"]
        ...
        #         contexts = documents
        #         answer = generation
        #     elif "transform_query" in step:
        #         transform_query = step["transform_query"]
        #         question = transform_query["question"]
        #         documents = transform_query["documents"]
